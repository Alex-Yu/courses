...
object Anagrams {
  ...
  def wordAnagrams(word: Word): List[Word] = (dictionaryByOccurrences compose wordOccurrences)(word)
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] = {
    def loop(tail: Occurrences): List[Occurrences] = tail match {
      case Nil => List(List.empty[(Char, Int)])
      case h :: t =>
        val (ch, freq) = h
        for {
          rest <- loop(t)
    ...
    loop(occurrences).map(_.filter(_._2 > 0))
  }
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences = y.foldLeft(x.toMap) {
    (z, el) =>
      val (ch, freq) = el
      val minuend = z(ch)
      require(minuend >= freq, "y occurrence is greater than x")
      if (minuend == freq) z - ch else z.updated(ch, minuend - freq)
  }.toList.sortBy(_._1)
